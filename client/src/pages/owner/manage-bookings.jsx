...
export default function ManageBookings() {
    ...
    useEffect(() => {
        if (bookings && bookings.length > 0) {
            ...
            Promise.all(customerIds.map(id => fetch(`/api/users/${id}`)
                .then(res => res.json())
                .catch(err => {
    ...
    const getCustomerDisplay = (customerId, booking) => {
    ...
    const handleUpdateBooking = () => {
        ...
        if (bookingNotes !== selectedBooking.notes)
            updateData.notes = bookingNotes;
    ...
    const BookingsTable = ({ bookings }) => (<div className="rounded-md border">
      <Table>
        <TableHeader>
          <TableRow>
        ...
        <TableBody>
          {bookings.length > 0 ? (bookings.map((booking) => (<TableRow key={booking.id}>
                ...
                <TableCell>
                  <Badge variant={booking.status === "confirmed" ? "outline" :
                ...
                setIsUpdateDialogOpen(true);
                    ...
                    {booking.status === "confirmed" && (<Button variant="ghost" size="icon" onClick={() => {
                    if (window.confirm("Are you sure you want to cancel this booking?")) {
                        deleteBookingMutation.mutate(booking.id);
                    }
                }} className="text-red-500 hover:text-red-700 hover:bg-red-50" disabled={deleteBookingMutation.isPending}>
        ...
        <main className="flex-1 p-4 md:p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <Input id="search" placeholder="Search by service ID, customer, mobile number..." className="pl-10" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}/>
                  </div>
                </div>
                <div>
          ...
          <Dialog open={isUpdateDialogOpen} onOpenChange={setIsUpdateDialogOpen}>
            <DialogContent className="sm:max-w-[550px]">
              ...
              {selectedBooking && (<div className="py-4">
                  ...
                  <div className="mb-4">
                    ...
                    <Textarea id="notes" value={bookingNotes} onChange={(e) => setBookingNotes(e.target.value)} className="mt-1" placeholder="Add notes about this booking" rows={3}/>
                  </div>
                </div>)}
