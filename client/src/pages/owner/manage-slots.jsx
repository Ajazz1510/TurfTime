    ...
    return (<div className="flex min-h-screen bg-background">
      ...
      <div className="flex-1 flex flex-col md:ml-64">
        ...
        <main className="flex-1 p-4 md:p-6 space-y-6">
          ...
          {turfs?.length === 0 && !turfsLoading && (<Card>
          ...
          {turfs && turfs.length > 0 && (<div className="flex flex-col space-y-4">
              <div className="flex flex-wrap gap-2">
                <Button variant={!selectedTurf ? "default" : "outline"} size="sm" onClick={() => setSelectedTurf("")}>
                  All Turfs
                </Button>
                
                {turfs.map((turf) => (<Button key={turf.id} variant={selectedTurf === turf.id.toString() ? "default" : "outline"} size="sm" onClick={() => setSelectedTurf(turf.id.toString())}>
                    {turf.name}
                  </Button>))}
              </div>
                  ...
                  {Object.entries(slotsByDate).map(([date, daySlots]) => (<Card key={date}>
                      ...
                      <CardContent className="p-0">
                        <Table>
                          ...
                          <TableBody>
                            {daySlots.map((slot) => (<TableRow key={slot.id}>
                                ...
                                <TableCell>
                                  {(() => {
                            const durationMinutes = Math.round((new Date(slot.endTime).getTime() - new Date(slot.startTime).getTime()) / 60000);
                            ...
                            return `${hours.toString().padStart(2, '0')}:${minutes.toString().padStart(2, '0')}`;
                                ...
                                <TableCell>
                                  {slot.isBooked ? (<span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-500">
                                      Booked
                                    </span>) : (<span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-500">
                                      Available
                                    </span>)}
                                </TableCell>
                                <TableCell className="text-right">
                                  {!slot.isBooked && (<Button variant="ghost" size="icon" onClick={() => deleteSlotMutation.mutate(slot.id)} disabled={deleteSlotMutation.isPending}>
