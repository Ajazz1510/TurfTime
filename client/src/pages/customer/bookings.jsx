...
import { sportTypeEnum } from "@shared/schema";
...
const getHourlyPrice = (sportType) => {
    switch (sportType) {
...
const calculateTotalPrice = (sportType, startTime, endTime) => {
...
const getDefaultPlayerCount = (sportType) => {
    switch (sportType) {
...
const generateTimeOptions = (startTime, endTime, intervalMinutes = 30) => {
    const options = [];
...
const formatTimeOption = (timeString) => {
...
export default function CustomerBookings() {
    ...
    const [createdBooking, setCreatedBooking] = useState(null); // To store newly created booking for payment
    ...
    const handleBookTurf = () => {
        if (!user || !selectedSlot || !selectedTurf)
            return;
    ...
    const filteredTurfs = turfs ? turfs.filter(turf => (turf.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    const filteredSlots = slots ? slots.filter(slot => selectedTurf ? slot.turfId === selectedTurf : true) : [];
        ...
        <main className="flex-1 p-4 md:p-6 space-y-6">
            ...
            <CardContent>
              <div className="flex flex-col md:flex-row gap-4">
                <div className="relative w-full md:w-1/2">
                  ...
                  <Input placeholder="Search by turf name or location..." className="pl-10" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}/>
                </div>
                <Select value={selectedSportType} onValueChange={(value) => setSelectedSportType(value)}>
                  <SelectTrigger className="w-full md:w-1/3">
                    <SelectValue placeholder="Filter by sport"/>
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="all">All sports</SelectItem>
                    {sportTypeEnum.enumValues.map((sport) => (<SelectItem key={sport} value={sport}>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            ...
            <TabsContent value="turfs" className="space-y-4 mt-6">
              {turfsLoading ? (<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                  {[1, 2, 3, 4, 5, 6].map((i) => (<Skeleton key={i} className="h-48 w-full"/>))}
                </div>) : filteredTurfs.length > 0 ? (<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                  {filteredTurfs.map((turf) => (<Card key={turf.id} className={`cursor-pointer transition-shadow hover:shadow-md ${selectedTurf === turf.id ? 'ring-2 ring-primary' : ''}`} onClick={() => {
                    ...
                    turf.sportType === "football" ? "outline" : "secondary"}>
                      ...
                      <CardContent>
                        <div className="flex flex-col space-y-3">
                          {turf.location && (<div className="flex items-center">
                      ...
                      <CardFooter>
                        <Button variant={selectedTurf === turf.id ? "default" : "outline"} className="w-full" onClick={() => {
                    const newSelectedTurf = selectedTurf === turf.id ? null : turf.id;
                    setSelectedTurf(newSelectedTurf);
                    if (newSelectedTurf) {
                        setActiveTab("slots");
              ...
              {selectedTurf && (<div className="mt-6 text-center">
            ...
            <TabsContent value="slots" className="space-y-4 mt-6">
              {selectedTurf ? (<>
                ...
                setActiveTab("turfs");
                  ...
                  {slotsLoading ? (<div className="grid gap-4 md:grid-cols-3">
                      {[1, 2, 3, 4, 5, 6].map((i) => (<Skeleton key={i} className="h-24 w-full"/>))}
                    </div>) : filteredSlots.length > 0 ? (<div className="grid gap-4 md:grid-cols-3">
                      {filteredSlots.map((slot) => (<Card key={slot.id} className={`cursor-pointer transition-shadow hover:shadow-md ${selectedSlot?.id === slot.id ? 'ring-2 ring-primary' : ''}`} onClick={() => setSelectedSlot(selectedSlot?.id === slot.id ? null : slot)}>
                          <CardContent className="p-4">
                            <div className="flex flex-col space-y-2">
                              ...
                              <Button variant={selectedSlot?.id === slot.id ? "default" : "outline"} size="sm" className="mt-2 w-full" onClick={(e) => {
                        e.stopPropagation();
          ...
          {(() => {
            ...
            return (<Dialog open={showPaymentDialog} onOpenChange={setShowPaymentDialog}>
                <DialogContent className="sm:max-w-[425px]">
                  ...
                  {createdBooking && (<div className="py-4">
                      ...
                      <div className="space-y-4">
                        ...
                        {selectedPaymentMethod === 'upi' && (<div className="space-y-2">
                        ...
                        {selectedPaymentMethod === 'card' && (<div className="space-y-2">
                        ...
                        {selectedPaymentMethod === 'netbanking' && (<div className="space-y-2">
                        ...
                        {selectedPaymentMethod === 'wallet' && (<div className="space-y-2">
                  ...
                  <DialogFooter>
                    ...
                    <Button onClick={handlePaymentSubmit} disabled={processPaymentMutation.isPending}>
            ...
            <DialogContent className="sm:max-w-[425px]">
              ...
              {selectedSlot && selectedTurf && (<>
                        ...
                        <div className="pt-2">
                          ...
                          <div className="grid grid-cols-2 gap-4 mt-2">
                            <div>
                              ...
                              <Select value={selectedStartTime ? format(selectedStartTime, 'HH:mm') : format(new Date(selectedSlot.startTime), 'HH:mm')} onValueChange={(time) => {
                ...
                while (currentTime < end) {
                    ...
                    options.push(<SelectItem key={`start-${timeStr}`} value={timeStr}>
                ...
                options.push(<SelectItem key={`start-${endTimeStr}`} value={endTimeStr}>
                                        {format(end, 'h:mm a')}
                                      </SelectItem>);
                return options;
            })()}
                            ...
                            <div>
                              ...
                              <Select value={selectedEndTime ? format(selectedEndTime, 'HH:mm') : format(new Date(selectedSlot.endTime), 'HH:mm')} onValueChange={(time) => {
                const [hours, minutes] = time.split(':').map(Number);
                const newDate = new Date(selectedSlot.startTime);
                newDate.setHours(hours, minutes);
                ...
                while (currentTime < end) {
                    const timeStr = format(currentTime, 'HH:mm');
                    options.push(<SelectItem key={`end-${timeStr}`} value={timeStr}>
                                          {format(currentTime, 'h:mm a')}
                                        </SelectItem>);
                    currentTime.setMinutes(currentTime.getMinutes() + 30);
                }
                // Add the end time as the last option
                const endTimeStr = format(end, 'HH:mm');
                options.push(<SelectItem key={`end-${endTimeStr}`} value={endTimeStr}>
                                        {format(end, 'h:mm a')}
                                      </SelectItem>);
                return options;
            })()}
                ...
                return (<div className="font-medium">
                    ...
                    <div className="space-y-1">
                      ...
                      <Input id="team-name" placeholder="Enter your team name" value={teamName} onChange={(e) => setTeamName(e.target.value)}/>
                    </div>
                    
                    <div className="space-y-1">
                      <Label htmlFor="player-count">Number of Players</Label>
                      <Input id="player-count" type="number" min={1} max={30} placeholder="Enter number of players" value={playerCount} onChange={(e) => setPlayerCount(parseInt(e.target.value) || 1)}/>
                    </div>
                    
                    <div className="space-y-1">
                      <Label htmlFor="mobile-number">Mobile Number (required)</Label>
                      <Input id="mobile-number" type="tel" placeholder="Enter 10-digit mobile number" value={mobileNumber} onChange={(e) => setMobileNumber(e.target.value)} required/>
                      {mobileNumber && mobileNumber.length < 10 && (<p className="text-sm text-destructive mt-1">Mobile number must be at least 10 digits</p>)}
                    </div>
                    
                    <div className="space-y-1">
                      <Label htmlFor="notes">Additional Notes (Optional)</Label>
                      <Textarea id="notes" placeholder="Any special requests or information for the turf owner" value={bookingNotes} onChange={(e) => setBookingNotes(e.target.value)}/>
                    </div>
                  </div>
                  ...
                  <DialogFooter>
                    ...
                    <Button onClick={handleBookTurf} disabled={createBookingMutation.isPending || !mobileNumber || mobileNumber.length < 10}>
