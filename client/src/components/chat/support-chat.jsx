      ...
      <div className="fixed bottom-6 right-6 z-50">
        <Sheet open={isOpen} onOpenChange={setIsOpen}>
          ...
          <SheetContent side="right" className="w-[90vw] sm:w-[450px] p-0 flex flex-col">
            ...
            <ScrollArea className="flex-1 px-4 py-3">
              <div className="flex flex-col gap-3">
                <AnimatePresence initial={false}>
                  {messages.map((message) => (<motion.div key={message.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.2 }} className={cn("flex flex-col max-w-[85%] rounded-lg px-3 py-2", message.sender === 'user' ? "ml-auto bg-primary text-primary-foreground" :
                message.sender === 'system' ? "mr-auto bg-muted" :
                    "mr-auto bg-muted")}>
                      <div className="flex items-center gap-1 mb-1">
                        {message.sender === 'user' ? (<User className="h-3 w-3"/>) : message.sender === 'ai' ? (<Bot className="h-3 w-3"/>) : (<Bot className="h-3 w-3"/>)}
                ...
                {aiResponseMutation.isPending && (<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center max-w-[85%] mr-auto bg-muted rounded-lg px-3 py-2">
            ...
            {/* Quick Responses */}
            {messages.length <= 2 && (<div className="px-4 py-2 border-t">
                <p className="text-xs text-muted-foreground mb-2">Quick questions:</p>
                <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-none">
                  {quickResponses.map((response) => (<Button key={response.id} variant="outline" size="sm" className="whitespace-nowrap" onClick={() => handleQuickResponseClick(response)}>
                      {response.text}
                    </Button>))}
                </div>
              </div>)}
            ...
            <div className="border-t p-4 mt-auto">
              <div className="flex gap-2">
                <Textarea ref={inputRef} value={inputValue} onChange={(e) => setInputValue(e.target.value)} placeholder="Type your message..." className="flex-1 resize-none min-h-[40px] max-h-[120px]" onKeyDown={(e) => {
            if (e.key === 'Enter' && !e.shiftKey) {
                e.preventDefault();
                handleSendMessage();
            }
        }}/>
                <Button onClick={handleSendMessage} disabled={!inputValue.trim() || aiResponseMutation.isPending} className="px-3 self-end h-[40px]">
