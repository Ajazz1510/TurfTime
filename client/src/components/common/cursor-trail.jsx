...
export default function CursorTrail({ className, color = "rgb(255, 255, 255)", // White color for particles
particleCount = 35, particleSize = 35, // Large particles
particleDecayRate = 0.03 // Faster decay rate for shorter trails
    ...
    useEffect(() => {
        ...
        let velocityY = 0;
        let counter = 0;
        const addPoint = (x, y, clientX, clientY) => {
    ...
    return (<div className={`pointer-events-none fixed inset-0 z-50 overflow-hidden ${className}`}>
      {/* Main cursor dot */}
      <motion.div className="absolute rounded-full bg-primary/60 blur-sm" style={{
      ...
      {points.map(point => {
            ...
            return (<motion.div key={point.id} className="absolute rounded-full blur-xl" style={{
                    left: point.x,
                    top: point.y,
