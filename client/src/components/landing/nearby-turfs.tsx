...
export default function NearbyTurfs() {
  ...
  const [nearbyTurfs, setNearbyTurfs] = useState<(Turf & {distance: number})[]>([]);
  ...
  useEffect(() => {
    if (userLocation && turfs && turfs.length > 0) {
      ...
      const turfsWithLocation = turfs.filter(
        turf => turf.latitude != null && turf.longitude != null
      );
  ...
  const getSportTypeStyles = (sportType: string) => {
    switch (sportType) {
      case "cricket":
  ...
  return (
    <section className="py-16 bg-black/40">
      <div className="container">
        ...
        {userLocation && nearbyTurfs.length === 0 && !isLoading && (
